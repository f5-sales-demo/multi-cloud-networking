...
# ruff: noqa: D103, S603, PLR2004, TRY004, TRY003, EM101
...
def output(terraform_dir: pathlib.Path, name: str) -> dict[str, Any]:
    value = json.loads(
        run(["terraform", f"-chdir={terraform_dir}", "output", "-json", name])
    )
    if not isinstance(value, dict):
        raise ValueError("Terraform output must be an object")
    return value
...
def main() -> None:
    ...
    check(
        re.fullmatch(r"[a-zA-Z0-9_.-]+", args.client_host) is not None,
        "invalid client host",
    )
    check(
        re.fullmatch(r"[a-zA-Z0-9_.-]+", args.client_interface) is not None,
        "invalid client interface",
    )
    ...
    check(
        isinstance(contract, dict) and contract.get("phase") == "configured",
        "KVM LAN is not configured",
    )
    check(
        status
        == {
    ...
    check(
        contract.get("bridge") == "br-kvm-lan" and contract.get("uplink") == "enp3s0",
        "shared bridge/uplink identity changed",
    )
    check(
        bool(contract.get("observed_slo"))
        and bool(contract.get("realized_sli"))
        and contract["observed_slo"] != contract["realized_sli"],
        "SLO and SLI identities are not distinct",
    )
    check(
        contract.get("observed_sli") == contract.get("realized_sli"),
        "provider SLI identity is inconsistent",
    )
    ...
    check(
        re.fullmatch(r"[a-z0-9.-]+", domain) is not None, "invalid application domain"
    )
    check(isinstance(port, int) and 1 <= port <= 65535, "invalid origin port")

    host_addr = json.loads(
        run(["ip", "-j", "address", "show", "dev", contract["bridge"]])
    )
    check(
        bool(host_addr) and host_addr[0].get("operstate") == "UP",
        "Ubuntu shared bridge is down",
    )
    check(
        "enp3s0"
        in run(
    ...
    client_addrs = remote(
        args.client_host, ["ip", "-4", "addr", "show", "dev", args.client_interface]
    )
    check(
        f"inet {client_ip}/24" in client_addrs,
        "independent client LAN allocation is missing",
    )
    route_to_vip = remote(args.client_host, ["ip", "route", "get", str(vip)])
    check(
        f"dev {args.client_interface}" in route_to_vip
        and f"src {client_ip}" in route_to_vip,
        "VIP path does not use the independent physical LAN client",
    )
    dns = remote(args.client_host, ["getent", "ahostsv4", domain])
    check(
        any(line.split()[0] == str(vip) for line in dns.splitlines()),
        "private application DNS does not resolve to the VIP",
    )
    origin_body = remote(
        args.client_host,
        ["curl", "-fsS", "--max-time", "15", f"http://{origin}:{port}/"],
        25,
    )
    check(
        "mcn-kvm-lan-origin" in origin_body,
        "Terraform-owned physical LAN origin marker is missing",
    )
    for _ in range(10):
        body = remote(
            args.client_host,
        ...
        check(
            "mcn-kvm-lan-origin" in body,
            "KVM inside VIP did not serve the owned origin",
        )
    route_after = remote(args.client_host, ["ip", "-4", "route", "show", "default"])
    check(
        route_before == route_after,
        "independent client default route changed during verification",
    )
    receipt = {
    ...
    print(
        "KVM LAN client: online=1 bgp=1 origin=1 vip_http=10/10 default_route=preserved"
    )
