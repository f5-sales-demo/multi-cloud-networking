...
KVM_PREFIXES = (
    "libvirt_", "docker_", "module.kvm_registration_mapping.",
    "xcsh_token.kvm", "xcsh_securemesh_site_v2.onprem_kvm",
    "xcsh_registration_approval.kvm", "xcsh_bgp.onprem_ebgp",
    "xcsh_smsv2_kvm_runtime_interface.", "xcsh_virtual_site.kvm_lan",
    "xcsh_origin_pool.kvm_lan", "xcsh_http_loadbalancer.kvm_lan",
    "terraform_data.kvm", "terraform_data.deployment_identity_guard",
)
AWS_PREFIXES = (
    "aws_", "module.aws_tgw_connect[", "xcsh_token.aws",
    "xcsh_securemesh_site_v2.aws", "xcsh_site_cloud_init.aws",
    "xcsh_registration_approval.aws", "xcsh_virtual_site.aws",
    "xcsh_origin_pool.aws", "xcsh_http_loadbalancer.aws",
    "xcsh_external_connector.aws_tgw", "xcsh_bgp.aws_tgw",
    "terraform_data.aws",
)
FULL_FLAGS = ("enable_azure", "enable_canada", "enable_bgp", "enable_azure_ilb", "enable_canada_ilb", "enable_kvm", "enable_kvm_lan", "enable_aws", "enable_aws_tgw_connect")
...
def validate(document: dict[str, Any], scope: str, source_commit: str,
    ...
    if xcsh.get("full_name") != "registry.terraform.io/f5-sales-demo/xcsh" or xcsh.get("version_constraint") not in ("11.3.0", "= 11.3.0"):
    ...
    output_changes = [
        name for name, item in (document.get("output_changes") or {}).items()
    ...
    if scope == "azure-approvals":
        ...
        approvals = [address for address, actions in changes
                     if address.startswith(("module.xc_site[", "module.xc_site_ca["))
                     and ".xcsh_registration_approval.this[" in address
                     and actions == ["create"]]
        if len(approvals) != 6:
            raise ValueError("Azure approval plan must create six registration approvals")
        for address, actions in changes:
            if address in approvals:
                continue
            if not (address.startswith(("module.xc_site[", "module.xc_site_ca["))
                    and ".xcsh_securemesh_site_v2.this[" in address
                    and actions == ["update"]):
                raise ValueError(f"Azure approval plan contains an action outside its scope: {address}")
        ...
        for address, actions in changes:
            if not address.startswith(AZURE_PREFIXES) or actions != ["update"]:
                raise ValueError(f"Azure convergence contains an action outside its update scope: {address}")
        ...
        for address, actions in changes:
            if not address.startswith(AZURE_PREFIXES):
                raise ValueError(f"Azure build contains an action outside its scope: {address}")
            if actions not in (["create"], ["update"]):
                raise ValueError(f"Azure build contains a destructive action: {address}")
    elif scope == "aws-kvm-build":
        if _value(document, "enable_azure") is not False or _value(document, "enable_canada") is not False:
            raise ValueError("AWS/KVM stage must disable Azure")
        if _value(document, "enable_kvm_lan") is not True or _value(document, "kvm_lan_configuration_phase") != "hardware":
        ...
        for address, actions in changes:
            if not address.startswith(AWS_PREFIXES + KVM_PREFIXES):
                raise ValueError(f"AWS/KVM build contains an action outside its scope: {address}")
            if actions not in (["create"], ["update"], ["delete"]):
                raise ValueError(f"AWS/KVM build contains an unsupported action: {address}")
        ...
        for address, actions in changes:
            if not address.startswith(KVM_PREFIXES):
                raise ValueError(f"KVM configuration contains an action outside its scope: {address}")
            if actions not in (["create"], ["update"], ["delete"]):
                raise ValueError(f"KVM configuration contains an unsupported action: {address}")
        ...
        if len(changes) != 1 or changes[0][0] not in (
            'aws_network_interface.slo[0]', 'libvirt_domain.ce_node["01"]'
        ...
        for address, actions in changes:
            ...
            if any(part in address for part in ("host_bridge", "shared_bridge", "uplink")):
            ...
            for item in document.get("resource_changes") or []:
                ...
                if marker.get("mcn_owner_id", marker.get("mcn-owner-id")) != owner_id or \
                   marker.get("mcn_environment", marker.get("mcn-environment")) != environment_key:
                    raise ValueError(f"destroy ownership marker mismatch: {item['address']}")
    elif scope == "zero-change":
        if changes or output_changes:
            raise ValueError(f"final plan is not zero-change: {changes[0][0] if changes else output_changes[0]}")
        if any(_value(document, name) is not True for name in FULL_FLAGS):
...
def main() -> int:
    ...
    parser.add_argument("--scope", choices=("aws-kvm-build", "aws-kvm-zero", "kvm-configured", "azure-build", "azure-approvals", "azure-converge", "refresh-only", "full-destroy", "zero-change"), required=True)
    ...
    backend_match = re.search(r'^\s*key\s*=\s*"([^"]+)"\s*$', args.backend_config.read_text(), re.M)
    ...
    result = subprocess.run(
        ["terraform", f"-chdir={args.terraform_dir}", "show", "-json", str(args.plan_file)],
        capture_output=True,
        check=True,
        timeout=120,
    )
