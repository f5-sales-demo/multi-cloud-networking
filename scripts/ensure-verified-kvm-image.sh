...
case "$url" in https://*) ;; *) printf 'image URL must use HTTPS\n' >&2; exit 65 ;; esac
case "$destination" in /*) ;; *) printf 'image destination must be absolute\n' >&2; exit 65 ;; esac

algorithm=${digest%%:*}
expected=${digest#*:}
[ "$algorithm:$expected" = "$digest" ] || { printf 'image digest must be ALGORITHM:HEX\n' >&2; exit 65; }
case "$algorithm" in
  md5) checker=md5sum; expression='^[0-9a-f]{32}$' ;;
  sha256) checker=sha256sum; expression='^[0-9a-f]{64}$' ;;
  sha512) checker=sha512sum; expression='^[0-9a-f]{128}$' ;;
  *) printf 'unsupported image digest algorithm: %s\n' "$algorithm" >&2; exit 65 ;;
esac
[[ "$expected" =~ $expression ]] || { printf 'invalid %s digest\n' "$algorithm" >&2; exit 65; }
...
matches_expected "$temporary" || { printf 'downloaded image digest mismatch for %s\n' "$url" >&2; exit 65; }
chmod 644 "$temporary"
