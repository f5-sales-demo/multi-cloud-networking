...
resource "terraform_data" "aws_tgw_contract_gate" {
  ...
  lifecycle {
    ...
    precondition {
      ...
      error_message = "Provider v11.0.1 must expose the exact immutable SMSv2 API v8.0.0 contract."
    }
    precondition {
      condition = (
      ...
      error_message = "Provider v11.0.1 must publish all and only the required SMSv2 capabilities, including evidence-backed AWS node configuration, as available."
    }
    precondition {
      condition = try(
