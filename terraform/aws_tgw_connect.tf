...
locals {
  ...
  aws_smsv2_api_release_commit = format("%s%s", "1c4f4eb8dd6cd9c440c2", "41b995a6c0ef1bcd23ab")
  ...
  lifecycle {
    ...
    precondition {
      condition = (
        ...
        data.xcsh_smsv2_contract.aws[0].api_release_tag == "v7.0.9" &&
      ...
      error_message = "Provider v11.0.0 must expose the exact immutable SMSv2 API v7.0.9 contract."
    }
    precondition {
      condition = (
      ...
      error_message = "Provider v11.0.0 must publish all and only the required SMSv2 capabilities, including evidence-backed AWS node configuration, as available."
    }
    precondition {
      condition = try(
