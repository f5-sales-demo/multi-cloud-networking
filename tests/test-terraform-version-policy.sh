...
echo "2. every xcsh consumer pins exactly v11.0.0"
for relative in terraform/versions.tf \
  ...
  if printf '%s\n' "$block" | grep -Eq 'version[[:space:]]*=[[:space:]]*"= 11\.0\.0"'; then
    ok "${relative} pins = 11.0.0"
  else
    bad "${relative} does not pin exactly = 11.0.0"
  fi
...
for relative in .github/workflows/terraform.yml prompt.txt docs/en/demo/deploy.mdx \
  ...
  if grep -Fq '11.0.0' "${REPO_ROOT}/${relative}"; then
    ok "${relative} references v11.0.0"
  else
    bad "${relative} is missing the v11.0.0 reference"
  fi
done
