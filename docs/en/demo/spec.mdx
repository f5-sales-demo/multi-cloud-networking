...
- **Base Image & Storage**: `libvirt_volume.base_cloud` imports only the F5-issued KVM appliance URL from `xcsh_site_image`; `libvirt_volume.ce_disk` provisions one 100 GiB root overlay disk (`onprem-ce-01`). The image query requires exactly one valid Sales Demo tenant `maurice_config`.
  With v10.1.0, the Site-UID resolver selects the current CE image before any libvirt mutation.
