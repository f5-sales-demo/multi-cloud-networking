...
1. **Versions & Providers** (`versions.tf`, `providers.tf`):
   ...
   - Providers: `f5-sales-demo/xcsh` (`= 11.3.0`, setting `api_url = local.xc_api_url` derived from
...
| --- | --- |
| `f5-sales-demo/xcsh = 11.3.0` | Provides the immutable SMSv2 contract, AWS node configuration, Azure NIC MAC-read stability, Customer Edge egress allowlists, and the Site-UID KVM image resolver. Azure multihop is implemented on the two FRR relays in each region. |
