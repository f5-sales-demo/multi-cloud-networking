...
2. Set `enable_kvm_lan = true`, set `kvm_lan_configuration_phase = "hardware"`,
   and supply the complete `kvm_lan` object. Leave `kvm_lan_observed_node = null`.
...
4. Apply that reviewed plan through the normal saved-plan path. After the rebuilt
   CE registers, correlate both devices to the Terraform-owned MACs. Do not guess
   `ethX` or reuse a prior registration suffix.
5. Set `kvm_lan_configuration_phase = "configured"` and supply the exact staged
   hostname, devices, and interface names in `kvm_lan_observed_node`. Review a
   second full-root plan. It may update only the owned site and create the owned
   KVM LAN virtual site, origin pool, and HTTP load balancer.
...
remain mandatory live acceptance evidence.
