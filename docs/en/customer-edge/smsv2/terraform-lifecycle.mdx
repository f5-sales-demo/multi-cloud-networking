...
Create-time software settings (`aws_software_version` and `aws_os_version`) are separate
from action targets (`aws_upgrade_software_version` and `aws_upgrade_os_version`). Action
targets default to create-time values; select newer targets only from a healthy site's
advertised eligible versions. Changing an action target never edits first-boot settings.
Provider v11 exposes
